...
if (isset($_SESSION['user_logged_in']) && $_SESSION['user_logged_in'] === true) {
    header('Location: garantia/garantias.php');
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    * {
    ...
    /* --- NOVOS ESTILOS PARA WIZARD/HINT --- */
    #hintWizard {
        position: absolute;
        top: -200%;
        left: 5%;
    ...
    .guide-hand {
        ...
        pointer-events: none;
        display: none;
        animation: handBounce 0.8s infinite alternate;
    }
...
<body>
    ...
    <div id="modalCadastro" class="modal">
        <div class="modal-content">
            <!-- Hint Wizard -->
            <div id="hintWizard">
                <span class="hint-close" id="closeHint">&times;</span>
                <p>Preencha o campo CNPJ e clique no botão de consulta, para que seja identificado seu cadastro em nosso ERP. Após isso será preenchido todos os campos, mude o login se desejar e coloque sua senha confirmando em seguida, caso seja Cliente já irá trazer o codigo do seu Representante Comercial, confirme os dados do formulário clicando em 'Cadastar' e você irá receber sua confirmação por email.</p>
            </div>
            
            <i class="fas fa-hand-point-right guide-hand" id="guideHand"></i>
            ...
            <form id="cadastroForm" style="max-height: 50vh; overflow-y: auto; padding-right: 15px;">
                ...
                <div class="input-group">
                    <input type="text" id="cadUsuario" name="usuario" required>
                    <label for="cadUsuario">Nome de Usuário</label>
                    <span id="userError" class="input-error-msg">O padrão é nome.sobrenome (sem espaços)</span>
                </div>
                <div class="input-group password-group">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Lógica do Wizard
        const hintWizard = document.getElementById('hintWizard');
        ...
        const cnpjSearchBtn = document.getElementById('cnpjSearchBtn');
        let hintTimeout;

        function positionHand(element) {
            if (!element) return;
            const rect = element.getBoundingClientRect();
            const modalRect = document.querySelector('#modalCadastro .modal-content').getBoundingClientRect();
            
            guideHand.style.display = 'block';
            guideHand.style.top = (rect.top - modalRect.top + (rect.height/4)) + 'px';
            guideHand.style.left = (rect.left - modalRect.left - 35) + 'px';
        }
        ...
        modalTriggers.forEach(trigger => {
            trigger.addEventListener('click', (e) => {
                ...
                // Se for o modal de cadastro, inicia o wizard
                if(modalId === '#modalCadastro') {
                    setTimeout(() => {
                        hintWizard.classList.add('active');
                    }, 500);

                    // Auto-fechar em 4 minutos
        ...
        function closeHint() {
            hintWizard.classList.remove('active');
            clearTimeout(hintTimeout);
            // Ao fechar o hint, mostra a mãozinha no CNPJ
            setTimeout(() => {
                positionHand(cadCnpj);
            }, 500);
        }

        closeHintBtn.addEventListener('click', closeHint);

        // Lógica da mãozinha seguindo o fluxo do CNPJ
        cadCnpj.addEventListener('input', () => {
            if(cadCnpj.value.length > 5) {
                positionHand(cnpjSearchBtn);
            } else {
                positionHand(cadCnpj);
            }
        });

        // Validação do campo Usuário (nome.sobrenome)
        const cadUsuario = document.getElementById('cadUsuario');
        const userError = document.getElementById('userError');

        cadUsuario.addEventListener('input', (e) => {
            let val = e.target.value;
            if (val.includes(' ')) {
                userError.style.display = 'block';
                // Remove espaços automaticamente ou substitui por ponto para ajudar o user
                e.target.value = val.replace(/\s/g, '.');
                showToast('Não é permitido espaços no usuário. Use ponto ( . )', 'warning');
            } else {
        ...
        document.getElementById('cadastroForm').addEventListener('submit', async (e) => {
            ...
            if (!data.senha || data.senha.trim() === "") {
                showToast('A senha não pode estar em branco.', 'error');
                return;
            }
        ...
        document.getElementById('cnpjSearchBtn').addEventListener('click', async (e) => {
            ...
            try {
                ...
                // Esconde a mãozinha após clicar na busca
                guideHand.style.display = 'none';
